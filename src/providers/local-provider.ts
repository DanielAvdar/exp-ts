...
export class LocalProvider implements LLMProvider {
    ...
    constructor() {
        this.settings = {
            endpoint: vscode.workspace.getConfiguration('vsc-chat.local').get('endpoint') || 'http://localhost:11434',
            model: 'llama3',
            temperature: 0.7,
            maxTokens: 1000
        };
    ...
    public async initialize(): Promise<void> {
        if (!this.settings.endpoint) {
            ...
            if (endpoint) {
                ...
                await vscode.workspace.getConfiguration('vsc-chat.local').update('endpoint', endpoint, true);
            } else {
    ...
    public async sendChatCompletionRequest(request: ChatCompletionRequest): Promise<ChatCompletionResponse> {
        try {
            if (!this.settings.endpoint) {
                await this.initialize();
            }
            ...
            const response = await axios.post(
                `${this.settings.endpoint}/api/chat`,
                {
            ...
            throw new Error(`Local LLM API error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
